...
namespace JSONFirebirdWebServiceTest.Exceptions
{
    public class ExceptionsLogByFile
    {
        ...
        public void LogException(Exception exc, String SQLCommand)
        {
            string ExceptionsFile = "";
            ...
            using (StreamWriter sw = File.AppendText(ExceptionsFile))
            {
                ...
                //Set SQL command
                string SQLText = "SQL COMMAND: ";
                SQLText = string.Concat(SQLText, SQLCommand);
                sw.WriteLine(SQLText, DateTime.Now);
